...
void server(int start_data[]){
    ...
    send_message(lb_sock,SERVER_UPDATE,temp,client_sock_addr,strlen(client_sock_addr));
    printf("connect LB:%d\n",res);
    ...
    { 
            ...
            if(FD_ISSET(lb_sock,&server_sockets)){
                struct message* curr_message_ptr=get_message(lb_sock);
            }
